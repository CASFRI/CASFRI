...
x = mutate(x, nat_non_veg=if_else(nat_non==" ","NULL_VALUE", if_else(!nat_non==" " & !nat_non %in% friList, "NOT_IN_SET", mapvalues(nat_non, friList, casList))))
```
...
x = unite(x, anth_veg_non, c(anth_veg, anth_non), sep="", remove=FALSE) %>% mutate(anth_veg_non = str_trim(anth_veg_non))
...
x = mutate(x, nat_non_veg=if_else(nfl=="","NULL_VALUE", if_else(!nfl=="" & !nfl %in% friList, "NOT_IN_SET", mapvalues(nfl, friList, casList))))
```
