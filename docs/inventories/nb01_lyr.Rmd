...
# Overview
  ...
  * The CAS_04 NB_0001.lyr file had several problems, so prior to using it I made the following modifications:
    ...
    * still leaves a discrepancy of 1 layer polygon
  * This cleaned up the data so that there were no longer "mixed" columns e.g., species code and percent
...
## productive_for {.tabset}
  ...
  * This one needs to be done properly...
  * Some obvious issues with CAS_04...
...
```{r echo=FALSE, message=FALSE, warning=FALSE}  
dfSummary(x4["PRODUCTIVE_FOR"], graph.col=FALSE, max.distinct.values=99)
```
...
## site_class {.tabset}
  ...
  * CASFRI specs (appendix 11) states that there is no field for site, so why is there a "P" value in CAS_04
  * CAS_04 again has mixed values in column

```{r echo=TRUE, message=FALSE, warning=FALSE}  
x = mutate(x, site_class=-8887)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}  
dfSummary(x4["SITE_CLASS"], graph.col=FALSE, max.distinct.values=99)
```
