:: Batch file for loading dependency validation tables into PostgreSQL
...
"%gdalFolder%/ogrinfo" PG:"host=%pghost% port=%pgport% dbname=%pgdbname% user=%pguser% password=%pgpassword%" -sql "CREATE SCHEMA IF NOT EXISTS %targetValidationFileSchema%";
...
(for %%f IN (%load_folders%) DO (
	if exist %%f (
		for %%g IN (%%f\*.csv) DO (
			...
			-nln %targetValidationFileSchema%.%%~ng ^
