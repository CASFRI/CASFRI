...
:: Each folder contains a shapefile and a .mdb database with a table of forest attributes and a 
:: table of non forest attributes.

:: Load the geometries, forest attributes and non forest attributes into three different tables 
...
SET inventoryID=ON01
SET srcFolder=%friDir%/ON/%inventoryID%/data/inventory/

SET fullTargetTableName=%targetFRISchema%.on01
SET tempPoly=%targetFRISchema%.on01_poly
SET tempForAtt=%targetFRISchema%.on01_forest_att
SET tempNonForAtt=%targetFRISchema%.on01_nonfor_att
...
:: drop any old temp tables
SET query_drop=DROP TABLE IF EXISTS %tempPoly%; ^
DROP TABLE IF EXISTS %tempForAtt%; ^
DROP TABLE IF EXISTS %tempNonForAtt%;

"%gdalFolder%\ogrinfo" %pg_connection_string% -sql "%query_drop%"
...
for %%F in (130 012 030 040 060 067 120 140 150 175 177 178 210 220 230 260 280 350 360 370 375 390 405 415 421 438 444 451 490 509 535 565 601 615 644 680 702 754 780 796 840 851 853 889 898 930 970 993) do (
  ...
  SET mdbForTableName=tbl_forest_%%F
  SET mdbNonForTableName=tbl_nonfor_%%F

  ECHO ############### Processing !shpName! ####################
...
:: Load non forest info from the .mdb using the ODBC driver
  ...
  SET overwrite_option=-update -append -addfields
  SET layer_creation_options=
)

ECHO ON

SET query=CREATE INDEX ON %tempForAtt% USING btree(src_filename); ^
