...
"%gdalFolder%/ogr2ogr" ^
-f "PostgreSQL" PG:"port=%pgport% host=%pghost% dbname=%pgdbname% user=%pguser% password=%pgpassword%" %srcFullPath%%srcFirstFileName%\forest ^
...
"%gdalFolder%/ogrinfo" PG:"port=%pgport% host=%pghost% dbname=%pgdbname% user=%pguser% password=%pgpassword%" -sql "DELETE FROM %fullTargetTableName%"
...
"%gdalFolder%/ogrinfo" PG:"port=%pgport% host=%pghost% dbname=%pgdbname% user=%pguser% password=%pgpassword%" -sql "ALTER TABLE %fullTargetTableName% ADD forest_id_1 integer;"
"%gdalFolder%/ogrinfo" PG:"port=%pgport% host=%pghost% dbname=%pgdbname% user=%pguser% password=%pgpassword%" -sql "ALTER TABLE %fullTargetTableName% ADD forest_id_2 integer;"
...
for /D %%F IN (%srcFullPath%\t*) DO (
	"%gdalFolder%/ogr2ogr" ^
	-update -append ^
	-f "PostgreSQL" PG:"port=%pgport% host=%pghost% dbname=%pgdbname% user=%pguser% password=%pgpassword%" "%%F\forest" ^
	-nln %fullTargetTableName% ^
	-t_srs %prjFile% ^
