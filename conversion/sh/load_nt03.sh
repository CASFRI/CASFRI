...
#
# Case 1 - Original polygon was intersected with a grid. Polygons intersecting 
#          different grid cells have the same fc_id and each part was assigned 
#          it's area after the intersection. All attributes except area and 
#          geometry are hence duplicated. We have to union them by fc_id and sum 
#          the areas to get the source area.
#
# Case 2 - Same as case 1 but the area values are for the full polygon before it 
#          was splitted by the grid. In this case we want to union by fc_id but 
#          keep the original area.
#
# Case 3 - Many rows are duplicated. The geometries and fc_id values are 
#          identical, but the other attributes are different without any reason. 
#          For example many cases have a sp3 percent value, but no other species 
#          info. Some rows have DST info which we should maintain. No rows have 
#          sp1 info. Many have NFL class. We should the row with the DST info if 
#          any is available. Otherwise the row selected should not influence the 
#          translation.
#
######################################## Set variables #######################################
